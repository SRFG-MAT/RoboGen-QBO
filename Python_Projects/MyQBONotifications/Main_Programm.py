...
# audio
sys.path.append('/opt/QBO/RoboGen-QBO/Python_Projects/EmotionAudio')
...
while True:
	...
	if firstRun == 0 or newSleepThreshold != sleepThreshold or newSleepMinValue != sleepMinValue or json.dumps(calendar) != json.dumps(newCalendar):
		...
		if sleepMinValue < sleepThreshold:
			Various_Functions.qboSpeak('Es scheint als haettest du kuerzlich schlecht geschlafen. Sage starte Entscheidungsbaum und anschließend Schlaf, um Hilfe zu bekommen.')
			
	# ToDo: Calculate actual calendar events and set timings accordingly
	if calendarTriggered == 0 and ts > (launch+30):
		calendarTriggered = 1
		Various_Functions.qboSpeak('In 10 Minuten ist Zeit für Medikamente nehmen.')
